	...
	if Patient then
		if Player.PlayerData.job.name == "ambulance" or QBCore.Functions.HasItem(src, "firstaid", 1) then
			...
			TriggerEvent('qb-log:server:CreateLog', 'ambulancejob', 'Player Banned', 'red',
				string.format('%s was banned by %s for %s', GetPlayerName(src), 'qb-ambulancejob',
					"Trying to revive theirselves or other players"), true)
...
QBCore.Commands.Add("localdoctor", "If EMS not online", {}, false, function(source, _)
	local src = source
	if Config.MinimalDoctors == 3 then
		local Player = QBCore.Functions.GetPlayer(src)

		-- if player not dead, don't do anything
		if not Player.Functions.GetMetaData("isdead") then
			return
		end

		-- if player is dead and has enough money, respawn them
		if Player.Functions.RemoveMoney("bank", Config.BillCost, "respawned-at-hospital") then
			...
			TriggerClientEvent('QBCore:Notify', src, "You do not have enough money to respawn!", "error")
		end
	else
		TriggerClientEvent('QBCore:Notify', src, "EMS has been notified!", "error")
	end
