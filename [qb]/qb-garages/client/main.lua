...
local function CreateGarageZone()
    ...
    combo:onPlayerInOut(function(isPointInside, l, zone)
        if isPointInside and IsAuthorizedToAccessGarage(zone.name) then
            ...
            -- exports["mv_interact"]:showInteraction('F1', Config.Garages[CurrentGarage]['drawText'], 'left')
            exports['qb-core']:DrawText(Config.HouseParkingDrawText, Config.DrawTextPosition)
        else
            exports['qb-core']:HideText()
            RemoveRadialOptions()
            CurrentGarage = nil
            -- exports["mv_interact"]:hideInteraction()
        end
    end)
end
...
local function RegisterHousePoly(house)
    ...
    zone:onPlayerInOut(function(isPointInside)
        if isPointInside then
            ...
            -- exports["mv_interact"]:showInteraction('F1', Config.HouseParkingDrawText, Config.DrawTextPosition)
...
CreateThread(function()
    for _, garage in pairs(Config.Garages) do
        if garage.showBlip then
            ...
            local blipColor = garage.blipColor ~= nil and garage.blipColor or 69
