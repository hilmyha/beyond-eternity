...
QBCore.Functions.CreateUseableItem('joint', function(source, item)
    ...
    -- set armor to 35
    -- TriggerClientEvent('hospital:server:SetArmor', source, 35)
    -- SetPedArmour(GetPlayerPed(source), 35)

    -- if job is police dont set armor
    if Player.PlayerData.job.name ~= 'police' then
        local armor = GetPedArmour(GetPlayerPed(source))
        SetPedArmour(GetPlayerPed(source), armor + 10)
    end
end)
...
QBCore.Functions.CreateUseableItem('armor', function(source)
    TriggerClientEvent('consumables:client:UseLightArmor', source)
end)
...
QBCore.Commands.Add('resetlightarmor', 'Resets Vest (Police Only)', {}, false, function(source)
    local Player = QBCore.Functions.GetPlayer(source)
    if Player.PlayerData.job.name == 'police' then
        TriggerClientEvent('consumables:client:ResetLightArmor', source)
    else
        TriggerClientEvent('QBCore:Notify', source, 'For Police Officer Only', 'error')
    end
end)

QBCore.Commands.Add('resetheavyarmor', 'Resets Vest (Police Only)', {}, false, function(source)
    local Player = QBCore.Functions.GetPlayer(source)
    if Player.PlayerData.job.name == 'police' then
        TriggerClientEvent('consumables:client:ResetHeavyArmor', source)
    else
        TriggerClientEvent('QBCore:Notify', source, 'For Police Officer Only', 'error')
    end
end)
...
RegisterNetEvent('consumables:server:resetHeavyArmor', function()
    local Player = QBCore.Functions.GetPlayer(source)
    if not Player then return end
    Player.Functions.AddItem('heavyarmor', 1)
end)

RegisterNetEvent('consumables:server:resetLightArmor', function()
    local Player = QBCore.Functions.GetPlayer(source)
    if not Player then return end
...
RegisterNetEvent('consumables:server:useLightArmor', function()
    local Player = QBCore.Functions.GetPlayer(source)
    if not Player then return end
