...
RegisterNetEvent('cdn-fuel:jerrycan:refuelvehicle', function(data)
	...
	if NotElectric then
		...
		if maxvehrefuel >= jerrycanfuelamount then maxvehrefuel = jerrycanfuelamount elseif maxvehrefuel < jerrycanfuelamount then maxvehrefuel =
			maxvehrefuel end
	...
	if Config.Ox.Input then
		...
		if refuel then
			...
			QBCore.Functions.Progressbar('refuel_gas', Lang:t("prog_jerry_can_refuel"), refueltimer, false, true,
				...
				if RefuelingType == nil then
					if Config.PlayerOwnedGasStationsEnabled and not Config.UnlimitedFuel then
						TriggerServerEvent('cdn-fuel:station:server:updatereserves', "remove", tonumber(refuel.amount), ReserveLevels,
							CurrentLocation)
						if CachedFuelPrice ~= nil then
							TriggerServerEvent('cdn-fuel:station:server:updatebalance', "add", tonumber(refuel.amount), StationBalance,
								CurrentLocation, CachedFuelPrice)
						else
							TriggerServerEvent('cdn-fuel:station:server:updatebalance', "add", tonumber(refuel.amount), StationBalance,
								CurrentLocation, FuelPrice)
				...
				if Config.Ox.Input then
					...
					if syphon then
						...
						if tonumber(syphonAmount) > maxsyphon then
							QBCore.Functions.Notify(Lang:t("syphon_kit_cannot_fit_1") .. fitamount .. Lang:t("syphon_kit_cannot_fit_2"),
								'error')
							return
						end
