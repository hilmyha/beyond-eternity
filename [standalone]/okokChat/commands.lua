	...
	RegisterCommand(Config.ClearEveryonesChatCommand, function(source, args, rawCommand)
		...
		if isAdmin(source) then
			...
			TriggerClientEvent('chat:addMessage', -1, {
				template =
				'<div class="chat-message system"><i class="fas fa-cog"></i> <b><span style="color: #df7b00">SYSTEM</span>&nbsp;<span style="font-size: 14px; color: #e1e1e1;">{0}</span></b><div style="margin-top: 5px; font-weight: 300;">Der Chat wurde gelöscht!</div></div>',
				args = { time }
			})
		end
	end)
...
if Config.EnableStaffCommand then
	RegisterCommand(Config.StaffCommand, function(source, args, rawCommand)
		...
		local time = os.date(Config.DateFormat)
		playerName = Config.ShowIDOnMessageForEveryone and xPlayer.PlayerData.charinfo.firstname.." "..xPlayer.PlayerData.charinfo.lastname.." ["..source.."]" or xPlayer.PlayerData.charinfo.firstname.." "..xPlayer.PlayerData.charinfo.lastname

		if isAdmin(source) then
			TriggerClientEvent('chat:addMessage', -1, {
				template =
				'<div class="chat-message staff"><i class="fas fa-shield-alt"></i> <b><span style="color: #bc1e29">[ADMIN] {0}</span>&nbsp;<span style="font-size: 14px; color: #e1e1e1;">{2}</span></b><div style="margin-top: 5px; font-weight: 300;">{1}</div></div>',
		...
		if isAdmin(source) then
			showOnlyForAdmins(function(admins)
				TriggerClientEvent('chat:addMessage', admins, {
					template =
					'<div class="chat-message staffonly"><i class="fas fa-eye-slash"></i> <b><span style="color: #1ebc62">[ADMIN CHAT] {0}</span>&nbsp;<span style="font-size: 14px; color: #e1e1e1;">{2}</span></b><div style="margin-top: 5px; font-weight: 300;">{1}</div></div>',
					args = { playerName, message, time }
				})
			end)
		end
	...
	RegisterCommand(Config.AdvertisementCommand, function(source, args, rawCommand)
		...
		if canAdvertise then
			if bankMoney >= Config.AdvertisementPrice then
				...
				TriggerClientEvent('okokNotify:Alert', source, "ADVERTISING",
					"Advertising made successful for " .. Config.AdvertisementPrice .. '€', 10000, 'success')
				...
				TriggerClientEvent('okokNotify:Alert', source, "ADVERTISING", "You don't have enough money to make an ad", 10000,
					'error')
			end
		else
			TriggerClientEvent('okokNotify:Alert', source, "ADVERTISING", "You can't advertise that quickly", 10000, 'error')
		end
	end)
end
...
if Config.EnablePoliceCommand then
	RegisterCommand(Config.PoliceCommand, function(source, args, rawCommand)
		...
		if job == Config.PoliceJobName then
			TriggerClientEvent('chat:addMessage', -1, {
				template =
				'<div class="chat-message police"><i class="fas fa-bullhorn"></i> <b><span style="color: #4a6cfd">{0}</span>&nbsp;<span style="font-size: 14px; color: #e1e1e1;">{2}</span></b><div style="margin-top: 5px;">{1}</div></div>',
				args = { playerName, message, time }
			})
		end
	end)
...
if Config.EnableMechanicCommand then
	RegisterCommand(Config.MechanicCommand, function(source, args, rawCommand)
		...
		if job == Config.MechanicJobName then
			TriggerClientEvent('chat:addMessage', -1, {
				template =
				'<div class="chat-message mechanic"><i class="fas fa-wrench"></i> <b><span style="color: #26A75D">{0}</span>&nbsp;<span style="font-size: 14px; color: #e1e1e1;">{2}</span></b><div style="margin-top: 5px; font-weight: 300;">{1}</div></div>',
				args = { playerName, message, time }
			})
		end
	end)
